<?php
    ...
    if ($action == 'fetch_post'){
        ...
        while ($postRecord = $execQuery->fetch_assoc()) {
            ...
            $cat_id = $postRecord['properties_id'];
            // Call the getCategory method to fetch the category using the category_id
            $postRecord['cat'] = getpropertiesById($cat_id, $conn);
    ...
    if ($action == 'fetch_a_post'){
        ...
        $cat_id = $post['properties_id'];
        $post['cat'] = getpropertiesById($cat_id, $conn);
    ...
    if ($authorized === false) {
        ...
        if ($action == 'save_post'){
            ...
            $data_insertion = $conn->query("INSERT INTO tbl_posts(properties_id, title, body, user_id, image) VALUES('$cat_id', '$title', '$body', $user_id, '$image')");
    ...
    function ImageUploader($file) {
        ...
        $filename =  time() . $name;
        $path = '../public/images/star1/' . $filename;
    ...
    function getpropertiesById($id, $conn) {
        $catQuery = $conn->query("SELECT * FROM tbl_properties  WHERE id = '$id'");
        $properties = $catQuery->fetch_assoc();
        return $properties;
    }
